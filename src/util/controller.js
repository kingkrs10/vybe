...
const credentialsFilePath = join(
  __dirname,
  `../../public/backup/${firbaseCredentialsFileName}`
);
...
const sendMail = async (request, response) => {
  try {
    ...
    firebaseAdmin
      .messaging()
      .sendToDevice(deviceToken, messageNotification, notification_options)
      .then((response) => {
    ...
    client.sendMail(mailOptionsInfo, (err, info) => {
      ...
      sendSuccessResponse(response, {
        message: `Mensagem enviada com sucesso. Id: ${info.messageId} | Response: ${info.response}`,
      });
    });
  } catch (error) {
    sendInternalErrorResponse(response, { message: error.toString() });
...
const migration = async (request, response) => {
  ...
  try {
    const filePath = join(
      __dirname,
      `../../public/backup/${firbaseDatabaseFileName}`
    );
    ...
    fs.readFile(filePath, "utf8", async (err, rawdata) => {
      ...
      try {
        ...
        const transactionHistoryData =
          jsonData[transaction_history_collection_name] || [];
        ...
        await qryClient.query(
          `INSERT INTO "currency"(id, "currencyDetails") VALUES($1, $2)`,
          [uuidv4(), [currencyData.currencyDetails]]
        );
        console.log("Done Currency saved sucessfully");

        await Promise.all(
          tempUserData.map(async (item, index) => {
            try {
              const value = userData[item];
              const checkAvailable = await qryClient.query(
                `SELECT "firebaseUId" FROM users WHERE "firebaseUId" = $1`,
                [item]
              );
              if (_isEmpty(checkAvailable.rows[0])) {
                ...
                if (value.blockListID && !_isEmpty(value.blockListID)) {
                  blockedUsersId.push({
                    firebaseUId: item,
                    userId: userId,
                    blockListID: value.blockListID,
                  });
                }
                if (value.inviteNumber && !_isEmpty(value.inviteNumber)) {
                  inviteNumbersArr.push({
                    firebaseUId: item,
                    userId: userId,
                    inviteNumber: value.inviteNumber,
                  });
                }
                const milliseconds = value["registrationDate"]
                  ? value["registrationDate"]["value"]["_seconds"] * 1000
                  : "";
                ...
                const reqObj = {
                  ...
                  thump_imageURL:
                    value["userTempImage"] || value["imageURl"] || null,
                  medium_imageURL:
                    value["userTempImage"] || value["imageURl"] || null,
                  stripeCustomerId: value["stripeCustomerId"] || "",
                  latitude: value["currentLocation"]
                    ? value["currentLocation"]["latitude"]
                    : "",
                  longitude: value["currentLocation"]
                    ? value["currentLocation"]["longitude"]
                    : "",
                  currencyCode: value["currencyDetails"]
                    ? value["currencyDetails"]["code"]
                    : "",
                  currencySymbol: value["currencyDetails"]
                ...
                await qryClient.query(
                  ...
                  [
                    userId,
                    reqObj.balance,
                    reqObj.notificationUnReadcount,
                    `{${reqObj.deviceId}}`,
                    reqObj.fullName,
                    `${reqObj.imageURl}`,
                    reqObj.phoneNumber,
                ...
                if (value.countryCurrency && !_isEmpty(value.countryCurrency)) {
                  await value.countryCurrency.map(async (items) => {
                    await qryClient.query(
                      `INSERT INTO "users_countryCurrency"("userId", amount, "oppPersonBalance", currency, label, value, "balanceData")
                           VALUES($1, $2, $3, $4, $5, $6, $7) RETURNING "userId"`,
                      [
                        userId,
                        items.amount,
                        items.oppPersonBalance,
                        items.currency,
                        items.label,
                        items.value,
                        items.balanceData,
        ...
        if (!_isEmpty(blockedUsersId)) {
          await Promise.all(
            blockedUsersId.map(async (item) => {
              item.blockListID.map(async (bUid) => {
                const userData = await qryClient.query(
                  `SELECT "userId", "firebaseUId" FROM users WHERE "firebaseUId" = $1`,
                  [bUid]
                );
                const blockedUserId = userData.rows[0]?.userId || null;
                if (blockedUserId) {
                  await qryClient.query(
                    `INSERT INTO "users_blockedUsers" ("userId", "blockedUserId") VALUES ($1, $2)`,
                    [item.userId, blockedUserId]
                  );
                }
              });
            })
        ...
        if (!_isEmpty(inviteNumbersArr)) {
          await Promise.all(
            inviteNumbersArr.map(async (item) => {
              item.inviteNumber.map(async (phoneNo) => {
                ...
                // if(invitedUserId){
                await qryClient.query(
                  `INSERT INTO users_invites("senderUserId", "receiverPhoneNumber", status)
                     VALUES($1, $2, $3)`,
                  [item.userId, phoneNo, 0]
                );
                // }
              });
            })
        ...
        await Promise.all(
          tempOfferData.map(async (item, index) => {
            try {
              const value = OfferData[item];
              // console.log('value',value);
              const checkAvailable = await qryClient.query(
                `SELECT "firebaseOfferId" FROM offers WHERE "firebaseOfferId" = $1`,
                [item]
              );
              const userData = await qryClient.query(
                `SELECT "userId", "firebaseUId" FROM users WHERE "firebaseUId" = $1`,
              ...
              if (_isEmpty(checkAvailable.rows[0])) {
                if (!_isEmpty(userData.rows[0])) {
                  ...
                  if (value.tempFavoriteID && !_isEmpty(value.tempFavoriteID)) {
                    offerFavoriteList.push({
                      firebaseOfferId: item,
                      offerId: offerId,
                      tempFavoriteIDs: value.tempFavoriteID,
                    });
                  }
                  if (value.reportListID && !_isEmpty(value.reportListID)) {
                    reportListIDsArr.push({
                      firebaseOfferId: item,
                      offerId: offerId,
                      reportListID: value.reportListID,
                    });
                  }
                  const transactionTime = value["createdAt"]
                    ? value["createdAt"]["value"]["_seconds"] * 1000
                    : "";
                  ...
                  await qryClient.query(
                    `INSERT INTO offers("offerId", "headLine", "offerImage", latitude, longitude, "offerDescription", "userId", "locationName", "firebaseOfferId", "createdAt", "offerThumpImage", "offerMediumImage")
                  ...
                  if (
                    value["offerHashTag"] &&
                    !_isEmpty(value["offerHashTag"])
                  ) {
                    qryClient.query(
                      `INSERT INTO "offers_hashTags"("offerId", "hashTag") VALUES ($1, $2)`,
                      [offerId, value["offerHashTag"]]
                    );
                  }
                } else {
        ...
        if (!_isEmpty(offerFavoriteList)) {
          await Promise.all(
            offerFavoriteList.map(async (item) => {
              item.tempFavoriteIDs.map(async (favoriteID) => {
                ...
                if (favoriteUId) {
                  const favoriteData = await qryClient.query(
                    `SELECT "userId", "offerId" FROM offers_favorites WHERE "offerId" = $1 AND "userId" = $2`,
                    [item.offerId, favoriteUId]
                  );
                  if (_isEmpty(favoriteData.rows[0])) {
                    await qryClient.query(
                      `INSERT INTO "offers_favorites" ("offerId", "userId") VALUES ($1, $2)`,
                      [item.offerId, favoriteUId]
                    );
                  }
                }
        ...
        if (!_isEmpty(reportListIDsArr)) {
          await Promise.all(
            reportListIDsArr.map(async (item) => {
              item.reportListID.map(async (reporterId) => {
                ...
                if (reporterUserId) {
                  await qryClient.query(
                    `INSERT INTO offers_reports("offerId", "reporterUserId") VALUES ($1, $2)`,
                    [item.offerId, reporterUserId]
                  );
                }
              });
            })
        ...
        console.log(
          "offers_favorites: total Records",
          offerFavoriteList.length
        );
        console.log("offers_reports: total Records", reportListIDsArr.length);

        // Transaction History Data Save Prcess.
        console.log(
          "Start Transaction data save process",
          temptransactionHistoryData.length
        );
        await Promise.all(
          temptransactionHistoryData.map(async (item, index) => {
            try {
              const value = transactionHistoryData[item];
              // console.log('temptransactionHistoryData', value);
              const senderUserData = await qryClient.query(
                `SELECT "userId", "currencyCode", "currencySymbol", "firebaseUId" FROM users WHERE "firebaseUId" = $1`,
                [value["sender_id"]]
              );
              const receiverUserData = await qryClient.query(
                `SELECT "userId", "firebaseUId" FROM users WHERE "firebaseUId" = $1`,
                [value["receiver_id"]]
              );
              const checkAvailable = await qryClient.query(
                `SELECT "firebaseTransactionId" FROM "transactionHistories" WHERE "firebaseTransactionId" = $1`,
                [item]
              );
              const senderData = senderUserData.rows[0] || null;
              const transactionTime = value["transaction_date"]
                ? value["transaction_date"]["value"]["_seconds"] * 1000
                : "";
              const createdAt = transactionTime
                ? new Date(transactionTime)
                : new Date();
              if (_isEmpty(checkAvailable.rows[0])) {
                if (
                  !_isEmpty(senderData) &&
                  !_isEmpty(receiverUserData.rows[0])
                ) {
                  ...
                  const reqObj = {
                    ...
                    senderCurrencyCode: value["senderSymbol"]
                      ? value["senderCurrency"]
                      : senderData.currencyCode
                      ? senderData.currencyCode
                      : null,
                    senderSymbol: value["senderSymbol"]
                      ? value["senderSymbol"]
                      : senderData["currencySymbol"]
                      ? senderData["currencySymbol"]
                      : null,
                    firebaseTransactionId: item,
                  };
                  await qryClient.query(
                    `INSERT INTO "transactionHistories" ("transactionId", amount, "senderUserId", "receiverUserId", "senderCurrencyCode", "senderSymbol", "firebaseTransactionId", "createdAt")
                    ...
                    [
                      ...
                      reqObj.senderUId,
                      reqObj.receiverUId,
                      reqObj.senderCurrencyCode,
                      reqObj.senderSymbol,
                      reqObj.firebaseTransactionId,
                      createdAt,
                    ]
                  );
                }
...
const fetchImages = async (collectionName) => {
  ...
  await files.map((item) => {
    if (
      !item.name.includes("_200x200") &&
      !item.name.includes("_400x400") &&
      !item.name.includes("_1080x1080")
    ) {
  ...
  await Promise.all(
    Images.map((item) => {
      ...
      if (
        item.includes(collectionName) &&
        !checkAvailable(`${tempFilename[0]}`)
      ) {
...
const firebaseImageDownload = async (request, response) => {
  ...
  try {
    ...
    await Promise.all(
      imagesData.map(async (file, index) => {
        const fileNameArr = file.split("/");
        const fileName = fileNameArr.pop();
        // DownLoad Source File
        downloadFilepath = join(
          workingDir,
          `${fileNameArr[1]}_temp_${fileName}`
        );
        try {
    ...
    const res = {
      message: "Complete Download Process",
      data: { success: downloadSuccess, fail: downloadFail },
    };
...
const firebaseImageResize = async (request, response) => {
  try {
    ...
    const res = {
      message: "done image resize process",
      data: { success: resizeSucc, failure: resizeFail },
    };
    sendSuccessResponse(response, res);
  } catch (error) {
    sendInternalErrorResponse(response, { message: error.toString() });
...
const firebaseImageUpload = async (request, response) => {
  try {
    ...
    const uploadFail = [];
    await Promise.all(
      imageData.map(async (file, index) => {
        const fileNameArr = file.split("/");
        const fileName = fileNameArr.pop();
        await Promise.all(
          [200, 400, 1080].map(async (size) => {
            ...
            const mimeType =
              tempFilename[1] === "png" ? "image/png" : "image/jpeg";
            try {
    ...
    const res = {
      message: "Done Upload process",
      data: { uploadSuccess, uploadFail },
    };
...
const firebaseImageDelete = async (request, response) => {
  ...
  try {
    ...
    const res = {
      message: "Done delete process",
      data: { success: success, failure: failure },
    };
    sendSuccessResponse(response, res);
  } catch (error) {
    sendInternalErrorResponse(response, { message: error.toString() });
...
const offerMigration = async (request, response) => {
  try {
    ...
    const offerData = await qryClient.query(
    ...
    if (offerData.rowCount > 0) {
      await Promise.all(
        offerData.rows.map(async (items, index) => {
