module.exports = {
    ...
    try {
      const result = await client.query(
        ...
        [
          ...
          reqObj.serviceName,
          reqObj.userId,
          reqObj.serviceImageURL,
          reqObj.serviceThumpImageURL,
          reqObj.serviceMediumImageURL,
          reqObj.serviceDescription,
          reqObj.categoryId,
          reqObj.subCategoryItemId,
          reqObj.locationName,
          reqObj.latitude,
          reqObj.longitude,
          reqObj.serviceStartingPrice,
          reqObj.paymentMethods
        ]
      );
    ...
    try {
      ...
      const result = await client.query(
        `SELECT
         ...
         LIMIT $3`,
        [true, reqObj.subCategoryItemId, limit]
      );
  ...
  remove: async (reqObj, client) => {
    try {
      const result = await client.query(
        `UPDATE services SET "isActive" = $1 WHERE "serviceId" = $2`,
        [false, reqObj.serviceId]
      );
